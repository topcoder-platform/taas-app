...
import { formatLocalTime, formatTimeOffset, formatWorkTime } from 'utils/format';
...
function TimeSection({ timeZone, start, end}) {
  ...
  const isWorkTime = () => {
    ...
    const startH = Number.parseInt(moment({hour: start.split(":")[0]}).format("H"), 10);
    let endH = Number.parseInt(moment({hour: end.split(":")[0]}).format("H"), 10);
    ...
    <div styleName="time-container">
      <div styleName="table-group">
        <div styleName="table-cell">
          {hasWorkTime() ? <span>WH: {formatWorkTime(start)} - {formatWorkTime(end)} {formatTimeOffset(tz)}</span> : "WH: Unknown" } 
        </div>
      </div>
      <div styleName="table-group">
        {hasWorkTime() && <div styleName="table-cell">
