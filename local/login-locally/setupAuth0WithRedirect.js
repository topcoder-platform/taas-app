...
script.src =
  "https://cdn.auth0.com/js/auth0-spa-js/1.10/auth0-spa-js.production.js";
...
const authSetup = function () {
          ...
          createAuth0Client({
            ...
            auth0
              ...
              .catch(function (e) {
                ...
                if (
                  e.error &&
                  (e.error == "login_required" || e.error == "timeout")
                ) {
      ...
      try {
        const storeRefreshedToken = function (aObj) {
          aObj
            ...
            .then(function (claims) {
              ...
              if (userActive) {
                ...
                try {
                  ...
                  logger(
                    "Error occured in fecthing token expiry time",
                    e.message
                  );
                  informIt(failed);
                }
            ...
            .catch(function (err) {
              logger(
                "Refeshed token - error in fetching token from auth0: ",
                err
              );
              informIt(failed);
            });
        ...
        const getToken = function (aObj) {
          aObj
            .getTokenSilently({ timeoutInSeconds: 60 })
            .then(function (token) {
              storeRefreshedToken(aObj);
            })
            .catch(function (err) {
              logger(
                "receiveMessage: Error in refreshing token through iframe:",
                err
              );
              informIt(failed);
            });
  ...
  function changeWindowMessage() {
    if (
      (!returnAppUrl && !appUrl) ||
      (returnAppUrl == "undefined" && appUrl == "undefined")
    ) {
      try {
        ...
        var linkurl =
          ...
          "/?logout=true&retUrl=http://" +
          window.location.host;
        if (hdomain) {
          linkurl =
            "https://" +
            window.location.host +
            "/?logout=true&retUrl=https://" +
            hdomain +
            ".com";
        }
        document.getElementById("page-title-heading").innerHTML = "Alert";
        document.getElementById("loading_message_p").innerHTML =
          "Login/Logout action is not called. Please check return url (retUrl) value in query parameters or <a href=" +
          linkurl +
          ">click here</a>";
      } catch (err) {
  ...
  function showLoginError(message, linkUrl) {
    try {
      document.getElementById("page-title-heading").innerHTML = "Alert";
      document.getElementById("loading_message_p").innerHTML =
        message + " <a href=" + linkUrl + ">click here</a>";
    } catch (err) {
      logger("Error in changing loading message: ", err.message);
    }
  }
